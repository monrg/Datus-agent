...
class DummyAgentConfig(AgentConfig):
    ...
    def sub_agent_config(self, sub_agent_name: str):
        return {"scoped_context": {"tables": "t1", "metrics": "m1", "sqls": "s1"}}
...
def test_sub_agent_instances_are_cached_per_name(tmp_path):
    ...
    assert first.db_path == str(tmp_path / "sub_agents" / "team_a")
    assert third.db_path == str(tmp_path / "sub_agents" / "team_b")
